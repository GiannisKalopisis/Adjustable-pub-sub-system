...
if __name__ == '__main__':
    ...
    if CROSS_VAL:
        ...
        lassocv = LassoCV(alphas=None, cv=percentage, fit_intercept=True, normalize=True, max_iter=100000, random_state=0)
        kf = KFold(n_splits=percentage, random_state=None, shuffle=True)

        # cross_validation
        print("Cross Validation (sklearn):")
        for score_param in scoring_dict:
            score = cross_val_score(lassocv, X.values, y.values.ravel(), scoring=scoring_dict[score_param], cv=kf)
            print("%s:" % (score_param))
            # print(score)
    ...
    # classic lasso
        ...
        print_metrics(y_test, lassocv.predict(X_test))
